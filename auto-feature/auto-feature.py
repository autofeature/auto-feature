    ...
    for cols in bin_cols:
        if len(bin_test[cols].dropna().unique())==2:
            if sum(bin_test[cols].isna())>len(bin_test[cols])*missing_threshold:
                ...
                dummy_temp=pd.get_dummies(bin_test[cols],prefix=cols,dummy_na=False)
                bin_test=pd.concat([bin_test,dummy_temp],axis=1)
    return bin_test         
...
def TnI_cat(testdata, x, y, missing_flag = True, missing_thres = 0.5, min_bin_size = 0.1,             binary_var = True,bv_min_size = 0.1, bv_min_incindex = 0.1, bar = False):
    ...
    if is_numeric_dtype(testdata['x_royalias']): sys.exit('ERROR!!! '+x+' variable is not string type. TnI_categorical requires x variable to be string, please check variable type')
    ...
    if is_string_dtype(testdata['y_royalias']): sys.exit('ERROR!!! y ('+y+') variable is not numeric or integer. TnI_categorical requires y variable to be numeric or integer, please check variable type')
    ...
    lookup_table=pd.DataFrame({'var_name':x,'original_name':x,'type':'original'},index=[0])
    ...
    if missing_flag==True:
        if testdata['x_royalias'].isna().mean()>0: # do we need mean here or count would do?
            ...
            lookup_table=pd.concat([lookup_table,pd.DataFrame({'var_name':x+'_missing_flag','original_name':x,'type':'missing_flag'},index=[1])])
    ...
    testlookup2['x_royalias'].replace('TnImissing',np.nan,inplace=True)
    lookup_table_rep=pd.DataFrame({'original_name':np.repeat(x,len(testlookup2)),'old_val':testlookup2['x_royalias'],'new_val':testlookup2['x_royassign']})
    ...
    testdata['x_royalias'].replace('TnImissing',np.nan,inplace=True)
    ...
    testdata['x_roybins_bi']=pd.cut(testdata['x_royassign'],cut_point, right=True)
...
def TnI_cont(testdata, x, y, missing_flag = True, missing_thres = 0.5, min_bin_size = 0.1, method = 'index',
    ...
    #Exit with error if the column data type is not string
    if is_string_dtype(testdata['x_royalias']): sys.exit('ERROR!!! '+x+' variable is not numeric. TnI_categorical requires x variable to be numeric, please check variable type')
    ...
    lookup_table_bin=pd.DataFrame({'var_name':x,'lower_bound':bnds[0],'upper_bound':bnds[1]},index=[0])
    testdata['x_royalias']=pd.Series(np.minimum(np.maximum(testdata['x_royalias'].to_numpy(),bnds[0]),bnds[1])) ## THIS NEEDS TO BE VERIFIED
    ...
    testdata['x_roybins_bi']=pd.cut(testdata['x_royalias'],cut_point, right=True)
    ...
    if testdata['x_royalias'].isna().mean()>0:
        if method=='index':
            lookup_table_rep=pd.DataFrame({'original_name':x,'old_val':np.NaN,'new_val':testdata['x_royalias'][testdata['y_roydistance']==testdata['y_roydistance'][testdata['y_roydistance']>0].min()].mean()},index=[0])
        ...
        if method=='index_med':
            lookup_table_rep=pd.DataFrame({'original_name':x,'old_val':np.NaN,'new_val':quantile_function(testdata['x_royalias'][testdata['y_roydistance']==testdata['y_roydistance'][testdata['y_roydistance']>0].min()],.5)},index=[0])
        ...
        if method=='mean':
            lookup_table_rep=pd.DataFrame({'original_name':x,'old_val':np.NaN,'new_val':testdata['x_royalias'].mean()},index=[0])
        ...
        if method=='median':
            lookup_table_rep=pd.DataFrame({'original_name':x,'old_val':np.NaN,'new_val':testdata['x_royalias'].median()},index=[0])
    ...
    if other_transform==True:
        ...
        if 'Exp' in transformations:
            testdata['x_roytran_TnI_exp']=np.exp((testdata['x_royalias']-testdata['x_royalias'].min())/(testdata['x_royalias'].max()-testdata['x_royalias'].min()))
...
def TnI_smart(test_df, var_list, y, binary_min_threshold = 0.05, missing_flag = True, min_bin_size = 0.1,
    ...
    if not isinstance(test_df,pd.DataFrame): sys.exit('ERROR!!! df is not a pandas dataframe, please please convert data into a dataframe')
    dv=test_df[y]
    ...
    int_var_names=list(pd.unique(int_var['var_name']))
    binary_var_names=list(pd.unique(binary_var2['var_name']))
    category_var_names=list(pd.unique(category_var['var_name']))
    ...
    if len(int_var_names)>0:
        ...
        if bar == False:
            ...
            lookup_table_cont=pd.concat(lookup_table_cont.values(), ignore_index=True)
            lookup_table_cont_bin=pd.concat(lookup_table_cont_bin.values(), ignore_index=True)
            lookup_table_cont_rep=pd.concat(lookup_table_cont_rep.values(), ignore_index=True)

    else:
        lookup_table_cont=pd.DataFrame()
        lookup_table_cont_bin=pd.DataFrame()
        lookup_table_cont_rep=pd.DataFrame()
    ...
    if len(category_var_names)>0:
        ...
        if bar == False:
            ...
            lookup_table_cat=pd.concat(lookup_table_cat.values(), ignore_index=True)
            lookup_table_cat_bin=pd.concat(lookup_table_cat_bin.values(), ignore_index=True)
            lookup_table_cat_rep=pd.concat(lookup_table_cat_rep.values(), ignore_index=True)

    else:
        lookup_table_cat=pd.DataFrame()
        lookup_table_cat_bin=pd.DataFrame()
        lookup_table_cat_rep=pd.DataFrame()
    
    
    cat_df=cat_df.drop([y],axis=1)
    smart_df=pd.concat([other_df, int_df, cat_df, binary_tni],ignore_index=True,axis=1)
    smart_df.columns=list(other_df.columns)+list(int_df.columns)+list(cat_df.columns)+list(binary_tni.columns)
    lookup_table=pd.concat([lookup_table_cont,lookup_table_cat], ignore_index=True)
    lookup_table_rep=pd.concat([lookup_table_cont_rep,lookup_table_cat_rep], ignore_index=True)
    lookup_table_bin=pd.concat([lookup_table_cont_bin,lookup_table_cat_bin], ignore_index=True)
